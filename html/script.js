...
function populateColors() {
    ...
    for (const [id, color] of Object.entries(colors)) {
        ...
        span.onclick = () => {
            ...
            $.post("https://renzu_vehicleshop/choosecolor", JSON.stringify({ id }))
        };
        div.append(span);
        colorsEl.append(div); 
    }
...
$(document).ready(function() {
    ...
    <header style="text-align:center;padding:5px;"><button onclick="choosecolor(`primary`)" class="confirm_out" style="background:#7631f6;color:#fff !important;border-radius: 10px;width: 30%;background: #7631f6;color: #fff !important;border-radius: 10px;width: 30%;/* padding-right: 20px; */height: 20px;font-size: 10px;margin-top: 1px !important;height: 100%;margin-right: 20px;">Elsődleges szín</button><button onclick="choosecolor(`secondary`)" class="confirm_out" style="background:#712fe9;color:#fff !important;border-radius: 10px;width: 30%;/* padding-right: 20px; */height: 20px;font-size: 10px;margin-top: 1px;height: 100%;">Másodlagos szín</button></header>\
  ...
  <div class="container">\
    <div class="right">\
      ...
      <div class="app">\
      <div style="position: absolute;top: -12%;right: 0%;width: 100%;height: 80px;background: linear-gradient(        90deg        , rgb(128 41 255 / 95%) 0%, rgb(81 11 140 / 77%) 100%);border-radius: 10px;z-index: -1;padding: 25px;text-align: center;font-size: 20px;color: #fff;border-style: solid;border-color: aliceblue;border-bottom-style: unset;border-right-style: unset;border-left-style: none;"><img style="display: block; height: 60px; position: absolute; top: 2%; left: 9%;" id="shopicon" src="https://i.imgur.com/05SLYUP.png"/> <span id="shopname">Vehicle Lists </span></div>\
        ...
        if(!itemDisabled && garage_id != 'impound') {
            ...
            $('#contentVehicle').append(`
                <div class="row spacebetween">
                    <span class="title">KEZELHETŐSÉG</span>
                ...
                <div class="row spacebetween">
                    <span class="title">VÉGSEBESSÉG</span>
                ...
                <div class="row spacebetween">
                    <span class="title">NYOMATÉK</span>
...
function BuyVehicle(n,c,p,b) {
    ...
    $('#closemenu').append(`
        ...
        <div class="modal-content">
            <p class="title">Vásárlás megerősítése:</p>
            <br>
            <p class="vehicle">Jármű: `+n+`</p>
            <p style="float:left;" class="vehicle">Osztály: `+c+`</p>
            <p style="float:right;" class="vehicle"> `+p+` `+currency+` </p>
            
        </div>

        <div class="modal-footer">
            <div class="wrapper">
                <input type="radio" name="personal" id="personal-1" checked>
                <input type="radio" name="personal" id="personal-2">
                <label for="personal-1" class="option personal-1">
                <div class="dot"></div>
                <span>Magánszemély</span>
                </label>
                <label for="personal-2" class="option personal-2">
                <div class="dot"></div>
                <span>Céges</span>
                </label>
            </div>
    ...
    `);
    $('input[type=radio][id=personal-1]').change(function() {
        if (this.value == 'on') {
            CurrentVehicle.personal = 'personal';
        }
    });
    $('input[type=radio][id=personal-2]').change(function() {
        if (this.value == 'on') {
            CurrentVehicle.personal = 'frakcio';
        }
    });
...
function TestDrive(n,c,p,m) {
    ...
    $('#closemenu').append(`
        <div class="background-circle"></div>
        <div class="modal-content">
            <p class="title">Ezt az autót viszed próbaútra:</p>
            <br>
            <p class="vehicle">Jármű: `+n+`</p>
            <p style="float:left;" class="vehicle">Osztály: `+c+`</p>
            <p style="float:right;" class="vehicle">Idő: 5 perc</p>
        </div>
...
(() => {
    ...
    renzu_vehicleshop.Open = function(data) {
        ...
        for(i = 0; i < (data.length); i++) {
            ...
            if(data[i].brand == "Prémium"){
                $(".app_inner").append('<label style="cursor:pointer;"><input false="" id="tab-'+ i +'" onclick="ShowVehicle('+i+')" name="buttons" type="radio"> <label for="tab-'+ i +'"> <div class="app_inner__tab"> <span style="position:absolute;top:4px;left:8px;font-size:8px;color:#b0b1b1;font-weight: 500;">Class: '+ data[i].category +'</span> <span style="position:absolute;top:4px;right:5px;font-weight: 700;font-size:15px;color:#fff;">Ár: '+ data[i].price +' PP</span><h2 style="font-size:11px !important;"> <i class="icon" style="right:100px;"><img style="height:20px;" src="https://cdn.discordapp.com/attachments/709992715303125023/813351303887192084/wheel.png"></i> '+ data[i].name +' </h2> <div class="tab_left"> <i class="big icon"><img class="imageborder" style="width: 120%;height:80px;height: 80px;border-radius: 10px;border-width: medium;border-color: #9962ff;border-style: groove;" onerror="this.src=`https://cdn.discordapp.com/attachments/709992715303125023/813351303887192084/wheel.png`;" src="' + imagecar + '"></i>   </div> <div class="tab_right"><div class="row" id="confirm" style="width: 100%;"> <button class="confirm_out" style="background:#712fe9;" onclick="TestDrive(`'+ data[i].name +'`,`'+ data[i].category +'`,`'+ data[i].price +'`,`'+ data[i].model +'`)"> Próba út </button> </div> </div> </div> </label></input></label>');    
            }else{
                $(".app_inner").append('<label style="cursor:pointer;"><input false="" id="tab-'+ i +'" onclick="ShowVehicle('+i+')" name="buttons" type="radio"> <label for="tab-'+ i +'"> <div class="app_inner__tab"> <span style="position:absolute;top:4px;left:8px;font-size:8px;color:#b0b1b1;font-weight: 500;">Class: '+ data[i].category +'</span> <span style="position:absolute;top:4px;right:5px;font-weight: 700;font-size:15px;color:#fff;">Ár: '+ data[i].price +' CS</span><h2 style="font-size:11px !important;"> <i class="icon" style="right:100px;"><img style="height:20px;" src="https://cdn.discordapp.com/attachments/709992715303125023/813351303887192084/wheel.png"></i> '+ data[i].name +' </h2> <div class="tab_left"> <i class="big icon"><img class="imageborder" style="width: 120%;height:80px;height: 80px;border-radius: 10px;border-width: medium;border-color: #9962ff;border-style: groove;" onerror="this.src=`https://cdn.discordapp.com/attachments/709992715303125023/813351303887192084/wheel.png`;" src="' + imagecar + '"></i>   </div> <div class="tab_right"> <button class="confirm_out" style="background:#712fe9" onclick="BuyVehicle(`'+ data[i].name +'`,`'+ data[i].category +'`,`'+ data[i].price +'`,`'+ data[i].brand +'`)"> Vásárlás </button> <div class="row" id="confirm" style="width: 100%;"> <button class="confirm_out" style="background:#712fe9;" onclick="TestDrive(`'+ data[i].name +'`,`'+ data[i].category +'`,`'+ data[i].price +'`,`'+ data[i].model +'`)"> Próba út </button> </div> </div> </div> </label></input></label>');    
            }
        }     
    }
    renzu_vehicleshop.Open(VehicleArr)
