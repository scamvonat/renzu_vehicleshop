...
local type = 'car'
local vehiclesdb = {}
presetprimarycolor = nil
presetsecondarycolor = nil
livery = nil
shopcoords = {}
brand = nil
shoptype = nil
...
CreateThread(function()
    ...
    for k, v in pairs(VehicleShop) do
        ...
        AddTextComponentString("Vehicle Shop: " .. v.name .. "")
...
function PopUI(name, v, event, text, server, shopname)
    ...
    local dist = #(v - GetEntityCoords(cache.ped))
    while dist < 5 and neargarage do
        dist = #(v - GetEntityCoords(cache.ped))
        Wait(100)
    end
...
function CreateShop(vec, msg, event, dist)
    local point = lib.points.new({ coords = vec, distance = dist, isOpen = false })
    ...
    function point:nearby()
        if IsControlJustPressed(0, 38) and not self.isOpen then
...
function OnExitRefund(self)
    lib.hideTextUI()
end

function OnEnterRefund(self)
    if not cache.vehicle then return end

    local vehProps = ESX.Game.GetVehicleProperties(cache.vehicle)
    local carPrice = lib.callback.await('renzu_vehicleshop:getCarPrice', false,
        GetDisplayName(vehProps.model):lower())

    if carPrice then
        lib.showTextUI("Nyomj [E]-t, hogy eladd az aktuális autód " ..
            carPrice .. " Csurintért!")
    end
end
...
function Inside(self)
    if IsControlJustPressed(0, 38) then
        ...
        print(GetDisplayName(model):lower())
        print(GetDisplayName(model))

        TriggerServerEvent("renzu_vehicleshop:sellvehicle", GetDisplayName(model):lower())
...
CreateThread(function()
    if Config.UsePopUI then
        for k, v in pairs(VehicleShop) do
            ...
            CreateShop(vec, "Nyomj [E]-t, hogy átnézd a katalógust", 'vehicleshop', 5)
        end

        if Config.EnableVehicleSelling then
            for k, v in pairs(Refund) do
                lib.zones.box({
                    coords = vector3(v.shop_x, v.shop_y, v.shop_z),
                    size = vec3(5, 4, 5),
                    debug = false,
                    onEnter = OnEnterRefund,
                    onExit = OnExitRefund,
                    inside = Inside
                })
            end
    ...
    if Config.DisplayCars then
        ...
        while true do
            for shop, v in pairs(VehicleShop) do
                ...
                if dist < 50 and not cache.vehicle and v.displaycars then
                    neargarage = true
                    if not cleared then
                    ...
                    for k, v in pairs(v.displaycars) do
                        if #(vector3(v.coord.x, v.coord.y, v.coord.z) - GetEntityCoords(cache.ped)) < 3 then
                            ...
                            if nearveh ~= 0 then
                                ...
                                if name == 'not found' then
                                    name = GetLabelText(GetDisplayName(GetEntityModel(nearveh)))
                                end
                                ...
                                if stats_show == nil or stats_show ~= nearveh then
                                    ...
                                    CreateThread(function()
                                        while nearveh ~= 0 do
                                            Wait(200)
                                            nearveh = GetClosestVehicle(GetEntityCoords(cache.ped), 3.000, 0, 70)
                                        end
                                    end)
                                    while nearveh ~= 0 do
        ...
        if not DoesEntityExist(vehiclenow) then
            if dist <= v.Dist and job then
                if Config.Licensed then
                    local hasLicense = lib.callback.await('esx_license:checkLicense', false, cache.serverId, 'drive')
                    ...
                    if hasLicense then
                        ...
                        ShowNotification("Nincs jogosítványod!")
                    end
                else
                    if PlayerData.job.name == v.job then
                        jobcar = v.job
...
RegisterCommand('carsale', function(source) -- Toggles Phone Via Command. Can coment out.
    TriggerServerEvent("renzu_vehicleshop:getCarPrice")
end, false)
...
function SetVehicleProp(vehicle, props)
    ...
    if DoesEntityExist(vehicle) then
        ...
        if props.fuelLevel then
            ...
            if DecorGetFloat(vehicle, '_FUEL_LEVEL') then DecorSetFloat(vehicle, '_FUEL_LEVEL', props.fuelLevel + 0.0) end
        end
    end
end
...
RegisterNUICallback("choosecolor", function(data, cb)
    if colortype == 'primary' then
        presetprimarycolor = tonumber(data.id)
        if LastVehicleFromGarage then
            local colorPrimary, colorSecondary = GetVehicleColours(LastVehicleFromGarage)
            SetVehicleColours(LastVehicleFromGarage, presetprimarycolor, colorSecondary)
        end
    else
        presetsecondarycolor = tonumber(data.id)
        if LastVehicleFromGarage then
            local colorPrimary, colorSecondary = GetVehicleColours(LastVehicleFromGarage)
            SetVehicleColours(LastVehicleFromGarage, colorPrimary, presetsecondarycolor)
        end
    end
    cb(true)
end)
...
RegisterNUICallback("choosebrands", function(data, cb)
    ...
    if cars > 0 then
            ...
            for k, v in pairs(VehicleShop) do
                ...
                if Config.UseArenaSpawn then
                    if dist <= 40.0 and id == v.name then
                        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", 2800.5966796875 - 5.5, -3799.7370605469,
                            122.41514587402, 360.00, 0.00, 0.00, 60.00, false, 0)
                    ...
                    if dist <= 40.0 and id == v.name then
                        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", v.shop_x - 5.0, v.shop_y - 3.0,
                            v.shop_z - 28.5, 360.00, 0.00, 0.00, 60.00, false, 0)
        ...
        ShowNotification("Nincs elérhető jármű")
    end
end)
...
PopulateVehicleshop = function(k)
    ...
    for _, value in pairs(tb) do
        ...
        if IsModelInCdimage(vehicleModel) then
            ...
            if vehname == nil then
                vehname = GetDisplayName(value.name)
            end
            local pmult, tmult, handling, brake = 1000, 800, GetPerformanceStats(vehicleModel).handling,
                GetPerformanceStats(vehicleModel).brakes
            if shoptype == 'boat' or shoptype == 'plane' then
                pmult, tmult, handling, brake = 10, 8, GetPerformanceStats(vehicleModel).handling * 0.1,
                    GetPerformanceStats(vehicleModel).brakes * 0.1
            end

            local img = 'https://cfx-nui-renzu_vehicleshop/imgs/uploads/' .. value.model .. '.jpg'
            --local img = 'https://cfx-nui-ox_inventory/web/images/cars/'..value.model..'.png'
...
function OpenShop(id)
    ...
    if cars > 0 then
        ...
        if not Config.Quickpick and type == 'car' then
            ...
            for k, v in pairs(VehicleShop) do
                ...
                if Config.UseArenaSpawn then
                    ...
                    if dist <= 40.0 and id == v.name then
                        shopcoords = vector3(v.shop_x, v.shop_y, v.shop_z)
                        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", 2800.5966796875 - 4.0, -3799.7370605469 -
                            3.0, 140.4514587402, 360.00, 0.00, 0.00, 60.00, false, 0)
            ...
            CreateThread(function()
                ...
                --SetEntityAlpha(cache.ped,1,true)
                while inGarage do
                    Wait(0)
                    if LastVehicleFromGarage ~= nil then
        ...
        FreezeEntityPosition(cache.ped, true)
        SetEntityCoords(cache.ped, shopcoords)
...
function GetVehicleLabel(vehicle)
    local vehicleLabel = string.lower(GetDisplayName(GetEntityModel(vehicle)))
    ...
    for k, v in pairs(VehicleShop) do
        ...
        if dist <= 40.0 and id == v.name then
            ...
            if not HasModelLoaded(hash) then
                ...
                BusyspinnerOff()
                Wait(10)
                AddTextEntry("CUSTOMLOADSTR", 'Jármű adatok letöltése..')
                ...
                while not HasModelLoaded(hash) do
                    ...
                    if c > 20000 then
                        BusyspinnerOff()
                        Wait(10)
                        AddTextEntry("CUSTOMLOADSTR", 'A jármű letöltése túl sokáig tartott, próbáld újra..')
                        BeginTextCommandBusyspinnerOn("CUSTOMLOADSTR")
                        EndTextCommandBusyspinnerOn(4)
                        Wait(2000)
                        break
            ...
            --TaskWarpPedIntoVehicle(cache.ped, LastVehicleFromGarage, -1)
            inShowRoom('enter')
        end
    end
end
...
function BuyVehicle(data, notregister)
    ...
    local plate = GenPlate(VehicleShop[data.shop].plateprefix or false)

    lib.requestModel(hash)

    SetEntityCoords(cache.ped, VehicleShop[data.shop].shop_x, VehicleShop[data.shop].shop_y,
        VehicleShop[data.shop].shop_z, false, false, false, true)
    Wait(0)
    v = CreateVehicle(hash, VehicleShop[data.shop].spawn_x, VehicleShop[data.shop].spawn_y,
        VehicleShop[data.shop].spawn_z, VehicleShop[data.shop].heading, 1, 1)
    veh = v
    while not DoesEntityExist(veh) do Wait(10) end
    SetVehicleNumberPlateText(v, string.gsub(tostring(plate), '^%s*(.-)%s*$', '%1'))

    if presetprimarycolor ~= nil then
        local colorPrimary, colorSecondary = GetVehicleColours(v)
        SetVehicleColours(v, presetprimarycolor, colorSecondary)
    end
    if presetsecondarycolor ~= nil then
        local colorPrimary, colorSecondary = GetVehicleColours(v)
        SetVehicleColours(v, colorPrimary, presetsecondarycolor)
    end
    ...
    lib.callback("renzu_vehicleshop:buyvehicle", false, function(canbuy)
        if canbuy then
            successbuy = canbuy
            for k, v in pairs(VehicleShop) do
                local dist = #(vector3(v.spawn_x, v.spawn_y, v.spawn_z) - GetEntityCoords(cache.ped))
                if dist <= 70.0 and id == k then
                    DoScreenFadeOut(333)
                    ReqAndDelete(LastVehicleFromGarage)
                    Wait(333)
                    SetEntityCoords(cache.ped, v.shop_x, v.shop_y, v.shop_z, false, false, false, true)
            ...
            CloseNui()
            ShowNotification("Sikeres vásárlás: Rendszám: " .. props.plate .. "")
    ...
    end, data.model, props, data.payment, jobcar, type, garage, notregister, personal)
    local counter = 0
...
RegisterNUICallback(
    "TestDriveCallback",
    function(data, cb)
        TriggerServerEvent("renzu_vehicleshop:setplayerbucket")
        local ped = cache.ped
        local props = nil
        local veh = nil
        local v = nil
        ...
        if Config.EnableTestDrive and type ~= 'plane' then
            ...
            lib.callback("renzu_vehicleshop:GenPlate", false, function(plate)
                local vec = {}
                if Config.UseArenaSpawn and type == 'car' then
                    vec = vector4(2954.2451171875, -3730.5334472656, 140.69311523438, 47.894153594971)
                else
                    vec = vector4(VehicleShop[data.shop].spawn_x, VehicleShop[data.shop].spawn_y,
                        VehicleShop[data.shop].spawn_z, VehicleShop[data.shop].heading)
                end
                ...
                TaskWarpPedIntoVehicle(cache.ped, veh, -1)
                DoScreenFadeIn(3000)
                ShowNotification("Próbaút elindítva")
                SetEntityAlpha(v, 255, false)
                ...
                --Wait(100)
                --SetVehicleStatus(GetVehiclePedIsIn(cache.ped))
                i = 0
                min = 0
                max = 10
                plus = 0
                drawtext = false
                oldcoord = vector3(VehicleShop[data.shop].shop_x, VehicleShop[data.shop].shop_y,
                    VehicleShop[data.shop].shop_z)
                indist = false
                ...
                Wait(1000)
                while count > 1 and IsPedInAnyVehicle(cache.ped) do
                    count = count - 1
                ...
                SetEntityCoords(cache.ped, oldcoord)
                while not HasCollisionLoadedAroundEntity(cache.ped) do Wait(0) end
                FreezeEntityPosition(cache.ped, false)
                testdrive = false
                presetsecondarycolor = nil
                presetprimarycolor = nil
                livery = nil
                TriggerServerEvent("renzu_vehicleshop:setdefaultbucket")
            end)
        else
            SendNUIMessage(
                {
...
CreateThread(function() --load IPL for Vehicleshop
    Wait(1000)
    CloseNui()
    RequestIpl('shr_int')
